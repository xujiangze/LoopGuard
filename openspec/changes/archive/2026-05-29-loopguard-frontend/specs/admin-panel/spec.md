...
### Requirement: 程序管理页面
系统 SHALL 提供程序管理页面（admin only），展示已注册程序列表，并提供注册新程序的表单。

#### Scenario: 查看程序列表
- **WHEN** admin 用户访问 `/admin/programs`
- **THEN** 页面展示所有已注册程序列表：项目/名称、二进制路径、审批人、启用状态、操作按钮

#### Scenario: 注册新程序
- **WHEN** admin 点击"注册新程序"并填写表单（项目名、程序名、二进制路径、审批人、超时、参数白名单）
- **THEN** 系统调用 `POST /api/v1/programs`，成功后刷新列表

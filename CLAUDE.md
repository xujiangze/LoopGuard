...
```
工单状态变更 ──→ WebhookService.Trigger ──→ 异步 goroutine ──→ 企业微信 Webhook API
                                                                      │
...
| `service` | 核心业务逻辑：TicketService（提交/dry-run/审批/执行）、ProgramService（注册/更新/文件上传）、WebhookService（企业微信通知触发/发送/投递记录） |
...
- `pages/`: LoginPage, TicketListPage, TicketDetailPage, ProgramPage, ProgramDetailPage, WebhookPage, UserPage, ApiKeyPage
